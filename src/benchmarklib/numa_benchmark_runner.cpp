...
void NumaBenchmarkRunner::_benchmark_permuted_query_sets() {
  // Init results
...
void NumaBenchmarkRunner::_execute_query(const NamedQuery& named_query) {
  const auto& name = named_query.first;
...
NumaBenchmarkRunner NumaBenchmarkRunner::create(const BenchmarkConfig& config, const std::string& table_path,
  ...
  return NumaBenchmarkRunner(config, queries, create_context(config));
}

std::vector<std::string> NumaBenchmarkRunner::_read_table_folder(const std::string& table_path) {
...
NamedQueries NumaBenchmarkRunner::_read_query_folder(const std::string& query_path) {
...
NamedQueries NumaBenchmarkRunner::_parse_query_file(const std::string& query_path) {
  auto query_id = 0u;
...
cxxopts::Options NumaBenchmarkRunner::get_basic_cli_options(const std::string& benchmark_name) {
...
nlohmann::json NumaBenchmarkRunner::create_context(const BenchmarkConfig& config) {
