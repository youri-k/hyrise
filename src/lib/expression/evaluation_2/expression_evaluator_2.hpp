...
#include "expression_result_2.hpp"
#include "expression_evaluator_range_2.hpp"
 ...
 *      - evaluate_expression_to_result(): result is a ExpressionResult2<>, one entry per row in the input Chunk, or a
 ...
 */
class ExpressionEvaluator2 final {
 public:
   ...
   */
  ExpressionEvaluator2(const std::shared_ptr<const Table>& table, const ChunkID chunk_id,
...
//  PosList evaluate_expression_to_pos_list(const AbstractExpression& expression);
  ...
  std::shared_ptr<ExpressionResult2<Result>> evaluate_expression_to_result(const AbstractExpression& expression);

  void evaluate_block_to_result(
    const AbstractExpression& expression,
    const BaseExpressionEvaluatorRange& range, const std::shared_ptr<BaseExpressionResult2>& result);
...
// private:
//  template <typename Result>
//  std::shared_ptr<ExpressionResult2<Result>> _evaluate_arithmetic_expression(const ArithmeticExpression& expression);
//
//  template <typename Result>
//  std::shared_ptr<ExpressionResult2<Result>> _evaluate_logical_expression(const LogicalExpression& expression);
//
//  template <typename Result>
//  std::shared_ptr<ExpressionResult2<Result>> _evaluate_predicate_expression(
//      const AbstractPredicateExpression& predicate_expression);
//
//  template <typename Result>
//  std::shared_ptr<ExpressionResult2<Result>> _evaluate_binary_predicate_expression(
//      const BinaryPredicateExpression& expression);
//
//  template <typename Result>
//  std::shared_ptr<ExpressionResult2<Result>> _evaluate_like_expression(const BinaryPredicateExpression& expression);
//
//  template <typename Result>
//  std::shared_ptr<ExpressionResult2<Result>> _evaluate_is_null_expression(const IsNullExpression& expression);
//
//  template <typename Result>
//  std::shared_ptr<ExpressionResult2<Result>> _evaluate_in_expression(const InExpression& in_expression);
//
//  template <typename Result>
//  std::shared_ptr<ExpressionResult2<Result>> _evaluate_subquery_expression(
...
//
//  template <typename Result>
//  std::shared_ptr<ExpressionResult2<Result>> _evaluate_column_expression(const PQPColumnExpression& column_expression);
//
//  template <typename Result>
//  std::shared_ptr<ExpressionResult2<Result>> _evaluate_case_expression(const CaseExpression& case_expression);
//
//  template <typename Result>
//  std::shared_ptr<ExpressionResult2<Result>> _evaluate_cast_expression(const CastExpression& cast_expression);
//
//  template <typename Result>
//  std::shared_ptr<ExpressionResult2<Result>> _evaluate_value_or_correlated_parameter_expression(
//      const AbstractExpression& expression);
//
//  template <typename Result>
//  std::shared_ptr<ExpressionResult2<Result>> _evaluate_function_expression(const FunctionExpression& expression);
//
//  template <typename Result>
//  std::shared_ptr<ExpressionResult2<Result>> _evaluate_extract_expression(const ExtractExpression& extract_expression);
//
//  template <typename Result>
//  std::shared_ptr<ExpressionResult2<Result>> _evaluate_unary_minus_expression(
//      const UnaryMinusExpression& unary_minus_expression);
//
//  template <size_t offset, size_t count>
//  std::shared_ptr<ExpressionResult2<pmr_string>> _evaluate_extract_substr(
//      const ExpressionResult2<pmr_string>& from_result);
//
//  template <typename Result>
//  std::shared_ptr<ExpressionResult2<Result>> _evaluate_exists_expression(const ExistsExpression& exists_expression);
//
//  // See docs for `_evaluate_default_null_logic()`
//  template <typename Result, typename Functor>
//  std::shared_ptr<ExpressionResult2<Result>> _evaluate_binary_with_default_null_logic(
...
//  template <typename Result, typename Functor>
//  std::shared_ptr<ExpressionResult2<Result>> _evaluate_binary_with_functor_based_null_logic(
//      const AbstractExpression& left_expression, const AbstractExpression& right_expression);
//
...
//
//  std::shared_ptr<ExpressionResult2<pmr_string>> _evaluate_substring(
//      const std::vector<std::shared_ptr<AbstractExpression>>& arguments);
//  std::shared_ptr<ExpressionResult2<pmr_string>> _evaluate_concatenate(
//      const std::vector<std::shared_ptr<AbstractExpression>>& arguments);
//
//  template <typename Result>
//  static std::vector<std::shared_ptr<ExpressionResult2<Result>>> _prune_tables_to_expression_results(
//      const std::vector<std::shared_ptr<const Table>>& tables);
  ...
  std::vector<std::shared_ptr<BaseExpressionResult2>> _segment_materializations;
