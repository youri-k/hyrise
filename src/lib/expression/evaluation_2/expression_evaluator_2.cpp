#include "expression_evaluator_2.hpp"
...
#include "expression_functors_2.hpp"
...
ExpressionEvaluator2::ExpressionEvaluator2(
...
std::shared_ptr<ExpressionResult2<Result>> ExpressionEvaluator2::evaluate_expression_to_result(const AbstractExpression& expression) {
  const auto block_size = 5'000u;
  const auto result = std::make_shared<ExpressionResult2<Result>>(_output_row_count, expression.is_nullable_on_pqp());

  for (auto begin = size_t{0}; begin < _output_row_count; ++begin) {
    const auto end = std::min(begin + block_size, _output_row_count);
    const auto range = BaseExpressionEvaluatorRange{begin, end, ExpressionEvaluatorRangeMode::All};

    evaluate_block_to_result(expression, range, result);
  }

  return result;
}

void ExpressionEvaluator2::evaluate_block_to_result(
  const AbstractExpression& expression,
  const BaseExpressionEvaluatorRange& range,
  const std::shared_ptr<BaseExpressionResult2>& base_result) {

  /**
   * Evaluate arguments
   */
  auto result_used_for_argument = false;
  auto argument_results = std::vector<std::shared_ptr<BaseExpressionResult2>>{expression.arguments.size()};
  for (auto argument_idx = size_t{0}; argument_idx < expression.arguments.size(); ++argument_idx) {
    const auto& argument = *expression.arguments[argument_idx];

    if (!result_used_for_argument && argument.data_type() == base_result->data_type && argument.is_nullable_on_pqp() == base_result->is_nullable()) {
      argument_results[argument_idx] = base_result;
      result_used_for_argument = true;
    } else {
      argument_results[argument_idx] = _allocate_result(argument.data_type(), argument.is_nullable_on_pqp(), range.end - range.begin);
    }

    evaluate_block_to_result(argument, range, argument_results[argument_idx]);
  }
  ...
  switch (expression.type) {
    case ExpressionType::Arithmetic: {
      ...
      resolve_data_types(arithmetic_expression.left_operand()->data_type(), arithmetic_expression.right_operand()->data_type(), [&](const auto left_operand_data_type_t, const auto right_operand_data_type_t) {
        using LeftOperandDataType = typename decltype(left_operand_data_type_t)::type;
        using RightOperandDataType = typename decltype(right_operand_data_type_t)::type;
        using ResultDataType = typename ArithmeticTraits<LeftOperandDataType, RightOperandDataType>::ResultDataType;

        const auto& left_operand_result = *std::static_pointer_cast<LeftOperandDataType>(argument_results[0]);
        const auto& right_operand_result = *std::static_pointer_cast<LeftOperandDataType>(argument_results[1]);
        const auto& result = *std::static_pointer_cast<ResultDataType>(base_result);

        _evaluate_arithmetic_expression(arithmetic_expression.arithmetic_operator, left_operand_result, right_operand_result, result, range);
      });
    } break;
    ...
    case ExpressionType::Predicate: {
      ...
      _evaluate_predicate_expression_to_block(predicate_expression.predicate_condition, )

      resolve_data_types(predicate_expression.left_operand()->data_type(), predicate_expression.right_operand()->data_type(), [&](const auto left_operand_data_type_t, const auto right_operand_data_type_t) {
        using LeftOperandDataType = typename decltype(left_operand_data_type_t)::type;
        using RightOperandDataType = typename decltype(right_operand_data_type_t)::type;
        using ResultDataType = typename ArithmeticTraits<LeftOperandDataType, RightOperandDataType>::ResultDataType;
...
//      Fail("ExpressionEvaluator2 doesn't support Aggregates, use the Aggregate Operator to compute them");
//
...
template <typename LeftOperandResult, typename RightOperandResult, typename Result, typename Range>
std::shared_ptr<ExpressionResult2<Result>> ExpressionEvaluator2::_evaluate_arithmetic_expression(
  const ArithmeticOperator arithmetic_operator,
  const LeftOperandResult& left_operand_result,
  const RightOperandResult& right_operand_result,
  const Result& result,
  const Range& range) {
  if constexpr (std::is_same_v<LeftOperandResult::ValueType, NullValue> || std::is_same_v<RightOperandResult::ValueType, NullValue>) {
    for (const auto offset : range) {
      result.set_null(offset, true);
    }
  } else {
    resolve_arithmetic_operator(arithmetic_operator, [&](const auto arithmetic_operator_functor) {
      for (const auto offset : range) {
        if (left_operand_result.is_null(offset) || right_operand_result.is_null(offset)) {
          result.set_null(offset, true);
        } else {
          result.set_value(offset, arithmetic_operator_functor(left_operand_result.value(offset), right_operand_result.value(offset)));
          result.set_null(offset, false);
        }
      }
    });
  }
...
//template <>
//std::shared_ptr<ExpressionResult2<ExpressionEvaluator2::Bool>>
//ExpressionEvaluator2::_evaluate_binary_predicate_expression<ExpressionEvaluator2::Bool>(
//    const BinaryPredicateExpression& expression) {
//  auto result = std::shared_ptr<ExpressionResult2<ExpressionEvaluator2::Bool>>{};
//
...
//    result = _evaluate_binary_with_default_null_logic<ExpressionEvaluator2::Bool, Evaluator>(left, right);  // NOLINT
//  });
//
//  return result;
//  // clang-format on
...
//
//template <typename Result>
//std::shared_ptr<ExpressionResult2<Result>> ExpressionEvaluator2::_evaluate_binary_predicate_expression(
//    const BinaryPredicateExpression& expression) {
...
//template <>
//std::shared_ptr<ExpressionResult2<ExpressionEvaluator2::Bool>>
//ExpressionEvaluator2::_evaluate_like_expression<ExpressionEvaluator2::Bool>(const BinaryPredicateExpression& expression) {
//  /**
...
//  auto result_values = std::vector<ExpressionEvaluator2::Bool>(result_size, 0);
//
//  /**
...
//
//  return std::make_shared<ExpressionResult2<ExpressionEvaluator2::Bool>>(std::move(result_values),
//                                                                       std::move(result_nulls));
//}
//
//template <typename Result>
//std::shared_ptr<ExpressionResult2<Result>> ExpressionEvaluator2::_evaluate_like_expression(
//    const BinaryPredicateExpression& expression) {
//  Fail("Can only evaluate predicates to bool");
//}
//
//template <>
//std::shared_ptr<ExpressionResult2<ExpressionEvaluator2::Bool>>
//ExpressionEvaluator2::_evaluate_is_null_expression<ExpressionEvaluator2::Bool>(const IsNullExpression& expression) {
//  std::vector<ExpressionEvaluator2::Bool> result_values;
//
...
//
//template <typename Result>
//std::shared_ptr<ExpressionResult2<Result>> ExpressionEvaluator2::_evaluate_is_null_expression(
...
//template <>
//std::shared_ptr<ExpressionResult2<ExpressionEvaluator2::Bool>>
//ExpressionEvaluator2::_evaluate_in_expression<ExpressionEvaluator2::Bool>(const InExpression& in_expression) {
...
//      return std::make_shared<ExpressionResult2<ExpressionEvaluator2::Bool>>(
//          std::vector<ExpressionEvaluator2::Bool>{in_expression.is_negated()});
//    }
//
//    if (left_expression.data_type() == DataType::Null) {
//      // `NULL [NOT] IN ...` is NULL
//      return std::make_shared<ExpressionResult2<ExpressionEvaluator2::Bool>>(std::vector<ExpressionEvaluator2::Bool>{0},
...
//    return evaluate_expression_to_result<ExpressionEvaluator2::Bool>(*rewrite_in_list_expression(in_expression));
//
...
//
//        if constexpr (EqualsEvaluator::supports_v<ExpressionEvaluator2::Bool, ValueDataType, SubqueryDataType>) {
...
//     */
//
//    return _evaluate_in_expression<ExpressionEvaluator2::Bool>(*std::make_shared<InExpression>(
...
//
//template <typename Result>
//std::shared_ptr<ExpressionResult2<Result>> ExpressionEvaluator2::_evaluate_in_expression(
...
//template <>
//std::shared_ptr<ExpressionResult2<ExpressionEvaluator2::Bool>>
//ExpressionEvaluator2::_evaluate_predicate_expression<ExpressionEvaluator2::Bool>(
...
//      return _evaluate_binary_predicate_expression<ExpressionEvaluator2::Bool>(
...
//      return evaluate_expression_to_result<ExpressionEvaluator2::Bool>(
//          *rewrite_between_expression(predicate_expression));
//
//    case PredicateCondition::In:
//    case PredicateCondition::NotIn:
//      return _evaluate_in_expression<ExpressionEvaluator2::Bool>(static_cast<const InExpression&>(predicate_expression));
//
//    case PredicateCondition::Like:
//    case PredicateCondition::NotLike:
//      return _evaluate_like_expression<ExpressionEvaluator2::Bool>(
//          static_cast<const BinaryPredicateExpression&>(predicate_expression));
//
//    case PredicateCondition::IsNull:
//    case PredicateCondition::IsNotNull:
//      return _evaluate_is_null_expression<ExpressionEvaluator2::Bool>(
...
//
//template <typename Result>
//std::shared_ptr<ExpressionResult2<Result>> ExpressionEvaluator2::_evaluate_predicate_expression(
//    const AbstractPredicateExpression& predicate_expression) {
//  Fail("Can only evaluate predicates to bool");
//}
//
//template <typename Result>
//std::shared_ptr<ExpressionResult2<Result>> ExpressionEvaluator2::_evaluate_column_expression(
...
//  return std::static_pointer_cast<ExpressionResult2<Result>>(_segment_materializations[column_expression.column_id]);
//}
//
//template <typename Result>
//std::shared_ptr<ExpressionResult2<Result>> ExpressionEvaluator2::_evaluate_case_expression(
//    const CaseExpression& case_expression) {
//  const auto when = evaluate_expression_to_result<ExpressionEvaluator2::Bool>(*case_expression.when());
//
...
//      });
//
//  return std::make_shared<ExpressionResult2<Result>>(std::move(values), std::move(nulls));
//}
//
//template <typename Result>
//std::shared_ptr<ExpressionResult2<Result>> ExpressionEvaluator2::_evaluate_cast_expression(
...
//template <>
//std::shared_ptr<ExpressionResult2<ExpressionEvaluator2::Bool>>
//ExpressionEvaluator2::_evaluate_exists_expression<ExpressionEvaluator2::Bool>(const ExistsExpression& exists_expression) {
//  const auto subquery_expression = std::dynamic_pointer_cast<PQPSubqueryExpression>(exists_expression.subquery());
//  Assert(subquery_expression, "Expected PQPSubqueryExpression");
//
//  const auto subquery_result_tables = _evaluate_subquery_expression_to_tables(*subquery_expression);
//
//  std::vector<ExpressionEvaluator2::Bool> result_values(subquery_result_tables.size());
//
...
//
//template <typename Result>
//std::shared_ptr<ExpressionResult2<Result>> ExpressionEvaluator2::_evaluate_exists_expression(
//    const ExistsExpression& exists_expression) {
//  Fail("Exists can only return bool");
//}
//
//template <typename Result>
//std::shared_ptr<ExpressionResult2<Result>> ExpressionEvaluator2::_evaluate_value_or_correlated_parameter_expression(
//    const AbstractExpression& expression) {
...
//    return std::make_shared<ExpressionResult2<Result>>(std::vector<Result>{{Result{}}}, nulls);
//  } else {
//    Assert(value.type() == typeid(Result), "Can't evaluate ValueExpression to requested type Result");
//    return std::make_shared<ExpressionResult2<Result>>(std::vector<Result>{{boost::get<Result>(value)}});
//  }
//}
//
//template <typename Result>
//std::shared_ptr<ExpressionResult2<Result>> ExpressionEvaluator2::_evaluate_function_expression(
...
//
//template <>
//std::shared_ptr<ExpressionResult2<pmr_string>> ExpressionEvaluator2::_evaluate_extract_expression<pmr_string>(
...
//
//template <typename Result>
//std::shared_ptr<ExpressionResult2<Result>> ExpressionEvaluator2::_evaluate_extract_expression(
//    const ExtractExpression& extract_expression) {
//  Fail("Only Strings (YYYY-MM-DD) supported for Dates right now");
//}
//
//template <size_t offset, size_t count>
//std::shared_ptr<ExpressionResult2<pmr_string>> ExpressionEvaluator2::_evaluate_extract_substr(
//    const ExpressionResult2<pmr_string>& from_result) {
//  std::shared_ptr<ExpressionResult2<pmr_string>> result;
//
...
//  });
//
//  return std::make_shared<ExpressionResult2<pmr_string>>(std::move(values), from_result.nulls);
//}
//
//template <typename Result>
//std::shared_ptr<ExpressionResult2<Result>> ExpressionEvaluator2::_evaluate_unary_minus_expression(
...
//
//template <typename Result>
//std::shared_ptr<ExpressionResult2<Result>> ExpressionEvaluator2::_evaluate_subquery_expression(
//    const PQPSubqueryExpression& subquery_expression) {
//  // One table per row. Each table should have a single row with a single value
//  const auto subquery_result_tables = _evaluate_subquery_expression_to_tables(subquery_expression);
//
//  // One ExpressionResult2<Result> per row. Each ExpressionResult2<Result> should have a single value
...
//  }
//
//  return std::make_shared<ExpressionResult2<Result>>(std::move(result_values), std::move(result_nulls));
//}
//
//std::vector<std::shared_ptr<const Table>> ExpressionEvaluator2::_evaluate_subquery_expression_to_tables(
...
//}
//
//std::shared_ptr<ExpressionEvaluator2::UncorrelatedSubqueryResults>
//ExpressionEvaluator2::populate_uncorrelated_subquery_results_cache(
//    const std::vector<std::shared_ptr<AbstractExpression>>& expressions) {
//  auto uncorrelated_subquery_results = std::make_shared<ExpressionEvaluator2::UncorrelatedSubqueryResults>();
//  auto evaluator = ExpressionEvaluator2{};
...
//}
//
//std::shared_ptr<const Table> ExpressionEvaluator2::_evaluate_subquery_expression_for_row(
...
//}
//
//std::shared_ptr<BaseValueSegment> ExpressionEvaluator2::evaluate_expression_to_segment(
//    const AbstractExpression& expression) {
...
//}
//
//PosList ExpressionEvaluator2::evaluate_expression_to_pos_list(const AbstractExpression& expression) {
//  /**
//   * Only Expressions returning a Bool can be evaluated to a PosList of matches.
//   *
//   * (Not)In and (Not)Like Expressions are evaluated by generating an ExpressionResult2 of booleans
...
//
//              if constexpr (ExpressionFunctorType::template supports<ExpressionEvaluator2::Bool, LeftDataType,
//                                                                     RightDataType>::value) {
//                for (auto chunk_offset = ChunkOffset{0}; chunk_offset < _output_row_count; ++chunk_offset) {
//                  if (left_result.is_null(chunk_offset) || right_result.is_null(chunk_offset)) continue;
//
//                  auto matches = ExpressionEvaluator2::Bool{0};
...
//          const auto result = evaluate_expression_to_result<ExpressionEvaluator2::Bool>(expression);
...
//    case ExpressionType::Value: {
//      const auto& value_expression = static_cast<const ValueExpression&>(expression);
//      Assert(value_expression.value.type() == typeid(ExpressionEvaluator2::Bool),
//             "Cannot evaluate non-boolean literal to PosList");
//      // TRUE literal returns the entire Chunk, FALSE literal returns empty PosList
//      if (boost::get<ExpressionEvaluator2::Bool>(value_expression.value) != 0) {
...
//template <>
//std::shared_ptr<ExpressionResult2<ExpressionEvaluator2::Bool>>
//ExpressionEvaluator2::_evaluate_logical_expression<ExpressionEvaluator2::Bool>(const LogicalExpression& expression) {
//  const auto& left = *expression.left_operand();
//  const auto& right = *expression.right_operand();
//
//  // clang-format off
//  switch (expression.logical_operator) {
//    case LogicalOperator::Or:  return _evaluate_binary_with_functor_based_null_logic<ExpressionEvaluator2::Bool, TernaryOrEvaluator>(left, right);  // NOLINT
//    case LogicalOperator::And: return _evaluate_binary_with_functor_based_null_logic<ExpressionEvaluator2::Bool, TernaryAndEvaluator>(left, right);  // NOLINT
//  }
//  // clang-format on
//
//  Fail("GCC thinks this is reachable");
...
//
//template <typename Result>
//std::shared_ptr<ExpressionResult2<Result>> ExpressionEvaluator2::_evaluate_logical_expression(
//    const LogicalExpression& expression) {
//  Fail("LogicalExpression can only output bool");
//}
//
//template <typename Result, typename Functor>
//std::shared_ptr<ExpressionResult2<Result>> ExpressionEvaluator2::_evaluate_binary_with_default_null_logic(
...
//
//template <typename Result, typename Functor>
//std::shared_ptr<ExpressionResult2<Result>> ExpressionEvaluator2::_evaluate_binary_with_functor_based_null_logic(
//    const AbstractExpression& left_expression, const AbstractExpression& right_expression) {
//  auto result = std::shared_ptr<ExpressionResult2<Result>>{};
//
...
//      }
//
//      result = std::make_shared<ExpressionResult2<Result>>(std::move(values), std::move(nulls));
//
//    } else {
//      Fail("BinaryOperation not supported on the requested DataTypes");
//    }
...
//
//template <typename Functor>
//void ExpressionEvaluator2::_resolve_to_expression_result_view(const AbstractExpression& expression, const Functor& fn) {
//  _resolve_to_expression_result(expression,
//                                [&](const auto& result) { result.as_view([&](const auto& view) { fn(view); }); });
//}
//
//template <typename Functor>
//void ExpressionEvaluator2::_resolve_to_expression_result_views(const AbstractExpression& left_expression,
...
//
//template <typename Functor>
//void ExpressionEvaluator2::_resolve_to_expression_results(const AbstractExpression& left_expression,
//                                                         const AbstractExpression& right_expression,
//                                                         const Functor& fn) {
...
//
//template <typename Functor>
//void ExpressionEvaluator2::_resolve_to_expression_result(const AbstractExpression& expression, const Functor& fn) {
//  Assert(expression.type != ExpressionType::List, "Can't resolve ListExpression to ExpressionResult2");
//
//  if (expression.data_type() == DataType::Null) {
//    // resolve_data_type() doesn't support Null, so we have handle it explicitly
//    ExpressionResult2<NullValue> null_value_result{{NullValue{}}, {true}};
//
...
//ChunkOffset ExpressionEvaluator2::_result_size(const RowCounts... row_counts) {
...
//}
//
//std::vector<bool> ExpressionEvaluator2::_evaluate_default_null_logic(const std::vector<bool>& left,
...
//}
//
//void ExpressionEvaluator2::_materialize_segment_if_not_yet_materialized(const ColumnID column_id) {
//  Assert(_chunk, "Cannot access columns in this Expression as it doesn't operate on a Table/Chunk");
//
...
//          std::make_shared<ExpressionResult2<ColumnDataType>>(std::move(values), std::move(nulls));
//
//    } else {
//      segment_iterate<ColumnDataType>(segment, [&](const auto& position) {
//        values[chunk_offset] = position.value();
...
//      });
//
//      _segment_materializations[column_id] = std::make_shared<ExpressionResult2<ColumnDataType>>(std::move(values));
//    }
//  });
//}
//
//std::shared_ptr<ExpressionResult2<pmr_string>> ExpressionEvaluator2::_evaluate_substring(
...
//  }
//
//  return std::make_shared<ExpressionResult2<pmr_string>>(result_values, result_nulls);
//}
//
//std::shared_ptr<ExpressionResult2<pmr_string>> ExpressionEvaluator2::_evaluate_concatenate(
//    const std::vector<std::shared_ptr<AbstractExpression>>& arguments) {
//  /**
//   * Emulates SQLite's CONCAT() - e.g. returning NULL once any argument is NULL
//   */
//
//  std::vector<std::shared_ptr<ExpressionResult2<pmr_string>>> argument_results;
...
//      auto null_value_result = ExpressionResult2<pmr_string>{{pmr_string{}}, {true}};
//      return std::make_shared<ExpressionResult2<pmr_string>>(null_value_result);
//    }
//
...
//  }
//
//  return std::make_shared<ExpressionResult2<pmr_string>>(std::move(result_values), std::move(result_nulls));
//}
//
//template <typename Result>
//std::vector<std::shared_ptr<ExpressionResult2<Result>>> ExpressionEvaluator2::_prune_tables_to_expression_results(
...
//   * an ExpressionResult2 and return the vector of resulting ExpressionResults.
//   */
//
//  std::vector<std::shared_ptr<ExpressionResult2<Result>>> results(tables.size());
//
...
//    }
//
//    results[table_idx] = std::make_shared<ExpressionResult2<Result>>(std::move(result_values), std::move(result_nulls));
//  }
//
//  return results;
//}
