...
struct PartitionedElement {
  PartitionedElement() : row_id(NULL_ROW_ID), value(TempType<T>()) {}
  PartitionedElement(RowID row, TempType<T> val) : row_id(row), value(val) {}
      ...
      resolve_segment_type<T>(*segment, [&, chunk_id](auto& typed_segment) {
        ...
        iterable.for_each([&, chunk_id](const auto& value) {
          if (!value.is_null() || consider_null_values) {
            Hash hashed_value{};
            if constexpr (std::is_same_v<HashedType, std::string> && std::is_same_v<std::decay_t<decltype(value.value())>, std::string_view>) {
              hashed_value = std::hash<std::string_view>{}(value.value());
            } else {
              hashed_value = std::hash<HashedType>{}(type_cast<HashedType>(value.value()));
            }
  ...
  for (ChunkID chunk_id{0}; chunk_id < chunk_offsets.size(); ++chunk_id) {
    jobs.emplace_back(std::make_shared<JobTask>([&, chunk_id]() {
      ...
      for (size_t chunk_offset = input_offset; chunk_offset < input_offset + input_size; ++chunk_offset) {
        ...
        size_t radix;
        if constexpr (std::is_same_v<HashedType, std::string> && std::is_same_v<std::decay_t<decltype(element.value)>, std::string_view>) {
          radix = std::hash<std::string_view>{}(element.value) & mask;
        } else {
          radix = std::hash<HashedType>{}(type_cast<HashedType>(element.value)) & mask;
        }
