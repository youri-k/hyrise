...
TEST_F(LQPTranslatorTest, JoinNodeToJoinHash) {
   ...
   * Build LQP and translate to PQP
   */
  auto join_node = JoinNode::make(JoinMode::Inner, equals_(int_float_a, int_float2_a), int_float_node, int_float2_node);
  ...
  ASSERT_TRUE(join_op);
  EXPECT_EQ(join_op->primary_predicate().column_ids, ColumnIDPair(ColumnID{0}, ColumnID{0}));
...
TEST_F(LQPTranslatorTest, JoinNodeToJoinSortMerge) {
   ...
   */
  auto join_node =
      JoinNode::make(JoinMode::FullOuter, greater_than_(int_float_a, int_float2_a), int_float_node, int_float2_node);
  const auto op = LQPTranslator{}.translate_node(join_node);
  ...
  ASSERT_TRUE(join_op);
  EXPECT_EQ(join_op->primary_predicate().column_ids, ColumnIDPair(ColumnID{0}, ColumnID{0}));
  EXPECT_EQ(join_op->primary_predicate().predicate_condition, PredicateCondition::GreaterThan);
  EXPECT_EQ(join_op->mode(), JoinMode::FullOuter);
}

TEST_F(LQPTranslatorTest, JoinNodeToFallbackJoinNestedLoop) {
  /**
   * Build LQP and translate to PQP
   */
  auto join_node =
      JoinNode::make(JoinMode::Inner, greater_than_(int_float_b, int_float2_a), int_float_node, int_float2_node);
  const auto op = LQPTranslator{}.translate_node(join_node);
