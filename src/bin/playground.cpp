...
void process_pqp(const std::shared_ptr<opossum::AbstractOperator> op) {
  ...
  std::cout << "Skipping PQP." << std::endl;
  return;
}

void extract_logical_query_plan_cache_data(const TableIdentifierMap& table_name_table_id_to_map,
                             const AttributeIdentifierMap& attribute_to_attribute_id_map) {
  ...
  for (const auto& [query_string, lqp_node] : SQLLogicalPlanCache::get()) {
    ...
    // `lqp_node = lqp_node` required as lqp_node is not a variable but coming from the structured bindings
    visit_lqp(lqp_node, [&, lqp_node = lqp_node](const auto& node) {
      if (node->type == LQPNodeType::Predicate) {
          ...
          visit_expression(el, [&](const auto& expression) {
            if (expression->type == ExpressionType::LQPColumn) {
              ...
              if (original_node->type == LQPNodeType::StoredTable) {
                ...
                std::cout << "Table: " << table_name << " - col: " << table->column_name(original_column_id) << std::endl;
        ...
        if (predicate_node->predicate()->type == ExpressionType::Predicate) {
          auto predicate_expression = std::dynamic_pointer_cast<AbstractPredicateExpression>(predicate_node->predicate());
          ...
          std::cout << "$$$$$$$$$$"
                    << table_statistics
                           ...
                           .row_count()
                    << std::endl;
          selectivity_independent =
              table_statistics
                  ->estimate_predicate(operator_predicates.value()[0].column_id,
                                       operator_predicates.value()[0].predicate_condition,
                                       operator_predicates.value()[0].value, operator_predicates.value()[0].value2)
                  .row_count() /
              row_count_current_table;
        ...
        if (table_name != "$&$&404" && column_name != "$&$&404" && selectivity >= 0 && selectivity_independent >= 0 &&
            row_count_current_table >= 0) {
          ...
          sequential_accesses_csv_file << predicate_condition_to_string.left.at(
                                              operator_predicates.value()[0].predicate_condition)
                                       << ",";
...
int main() {
  ...
  // bool cache_is_gdfs = false;
  // try {
  //  auto& gdfs_cache = dynamic_cast<GDFSCache<std::shared_ptr<AbstractOperator>, std::string>&>(SQLLogicalPlanCache::get().cache());
  //  cache_is_gdfs = true;
  // } catch(const std::bad_cast&) {}

  // get "workload"
  extract_logical_query_plan_cache_data(table_name_table_id_to_map, attribute_to_attribute_id_map);
